...
\begin{center}
\includegraphics[width=0.75\textwidth]{figures/experiment-utils.png}
\end{center}

\textbf{Key components:}
\begin{itemize}
    \item \texttt{ExperimentAnalyzer} - A/B test analysis, IV, IPW, regression adjustment, retrodesign
...
Your data has \blue{solid validity} for prediction --- then you \red{step off a cliff} the moment you use it to justify an intervention.
...
$P(Y \mid do(X))$ --- \textit{``What happens if we intervene?''}  \\
\orange{\textbf{Business decisions live here}}
...
\begin{itemize}\setlength{\itemsep}{0.05cm}
    ...
    \item \orange{\textbf{Tools:}} DiD, IV, IPW, regression discontinuity, synthetic control
...
\vspace{0.1cm}
\begin{itemize}\setlength{\itemsep}{0.05cm}
    \item Dr. X got the new onboarding flow $\rightarrow$ 8 bookings/week
    \item We \red{cannot} observe: same doctor with the old flow $\rightarrow$ \textbf{?}
    \item \red{Individual causal effects are unobservable}
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Random treatment assignment creates \orange{exchangeable} groups
    \end{itemize}
...
\textbf{Power} = Probability of detecting an effect when it exists
...
\textbf{Input we need to calculate power:}
\begin{itemize}
    \item \red{\textbf{Expected effect size}} \emoji{person-climbing}: Magnitude of difference
    \item \textbf{Variance}: Variability of the outcome metric
    \item \textbf{Significance level}: Threshold for statistical significance
...
\textbf{MDE} = \blue{Smallest effect size} reliably detected at 80\% power, $\alpha$ = 0.05, and sample $n$.

\vspace{0.05cm}
\textbf{\orange{How to define MDE:}}
\begin{enumerate}
 \vspace{0.1cm}
    \item \textbf{Historical:} Based on past experiments and benchmarks (typically 1-5\%)
    \item \textbf{Resource-constrained:} Achievable with sample (e.g., 50K users $\rightarrow$ 3\% MDE)
\end{enumerate}

\vspace{0.1cm}
\begin{alertblock}{If MDE is unreasonable \& sample is limited:}
\vspace{0.2cm}
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{\orange{Accumulate evidence}}: Meta-analysis (pooled experiments)
    \item \textbf{\red{Go quasi-experimental}}: DiD, IPW, Synthetic Control, Geo-experiments, Interrupted Time Series
\end{enumerate}
...
\textbf{Problem:} Every test has a 5\% false positive rate by chance. Testing $m$ metrics/variants simultaneously inflates this:
...
\texttt{PowerSim} uses a simulation approach, more useful for complex designs and metrics (compliance, multiple variants, etc.)
...
Using sample size 10778 (driven by (1, 2))
Optimized sample sizes: {'control': 1167, 'variant_1': 4805, 'variant_2': 4805}
Achieved power: {'(0, 1)': 0.815, '(0, 2)': 0.994, '(1, 2)': 0.808}
...
\begin{itemize}
    ...
    \item Normality tests are too sensitive; \orange{sampling distribution} matters, not population
\end{itemize}
...
\textbf{Problem:} Significant results often \red{overestimate} the true effect

\vspace{0.2cm}
\textbf{Why?}
\begin{itemize}
    \item Selection bias: Only "winners" reported
    \item Small samples + low power $\rightarrow$ greater exaggeration (2-3x for power < 0.50)
\end{itemize}
\vspace{0.2cm}
...
\begin{itemize}
    ...
    \item Attendees are more engaged, higher baseline revenue (\red{confounding})
\end{itemize}
...
\textbf{Naive analysis:} Compare attendees vs non-attendees
...
\begin{itemize}
    ...
    \item \textbf{Target:} Shift estimand from ATE $\rightarrow$ ATT / ATO
\end{itemize}

\begin{center}
\begin{tabular}{cc}
\includegraphics[width=0.35\textwidth]{figures/bad-overlap.png} &
\includegraphics[width=0.35\textwidth]{figures/ipw-overlap.png} \\
{\small Bad overlap} & {\small Good overlap}
\end{tabular}
\end{center}
\end{frame}
...
\textbf{IV:} Use assignment as instrument to isolate the effect on compliers (attendees)\\
...
0  conversion          5.0          0.05144        0.011513  0.000008
